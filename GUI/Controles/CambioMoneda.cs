...
namespace Sistema_Bancario.Controles
{
   public partial class CambioMoneda : UserControl
   {
      ...
      int i;
      int i2;
      public CambioMoneda(ISession x_session)
      {
         if (x_session != null)
         {
            InitializeComponent();
      ...
      private void Cambiar_Click(object sender, EventArgs e)
      {
         ...
         if (!string.IsNullOrEmpty(tipoMoneda1.CboMoneda.Text.Trim()))
         {
            tipo.MonedaE = tipoMoneda1.CboMoneda.Text.Trim();
         }
         else { return; }

         if (!string.IsNullOrEmpty(monto1.TBMonto.Text.Trim()))
         {
            tipo.MontoE = Convert.ToDecimal(monto1.TBMonto.Text.Trim());
         }
         else { return; }

         if (!string.IsNullOrEmpty(txtConversion.Text.Trim()))
         {
            tipo.MontoS = Convert.ToDecimal(txtConversion.Text.Trim());
         }
         else { return; }

         if (!string.IsNullOrEmpty(tipoMoneda2.CboMoneda.Text.Trim()))
         {
            tipo.MonedaS = tipoMoneda2.CboMoneda.Text.Trim();
         }
         else { return; }

         tipo.Usuario = Session.UserNombreCompleto;
         if (movimiento.MovimientoTipoCambioInsert(tipo) > 0)
         {
            MessageBox.Show("Se realizo el Cambio");
         }
         else
            MessageBox.Show("No se puedo realizar el cambio");
      }
   }
}
